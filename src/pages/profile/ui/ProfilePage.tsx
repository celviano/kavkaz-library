...
export const ProfilePage = memo(() => {
  ...
  // Determine mode: seller if has any books
  const hasSellActivity = myBooks.length > 0
  const activeBooks     = myBooks.filter((b) => b.status === 'active')
  const pendingBooks    = myBooks.filter((b) => b.status === 'pending')

  // ── Seller badge ──────────────────────────────────────────────────────────
  const sellerBadge = hasSellActivity ? (
  ...
  // ── Seller extra info ─────────────────────────────────────────────────────
  const sellerExtraInfo = hasSellActivity && user.email ? (
    <ProfileInfoRow icon={<IconMail />} label="Email" value={user.email} />
  ) : null

  // ── Stats strip ───────────────────────────────────────────────────────────
  const statsNode = hasSellActivity ? (
    <UserStatsStrip
      dark
      cols={4}
      stats={[
  ...
  // ── Trust pills (seller only) ─────────────────────────────────────────────
  const trustNode = hasSellActivity && stats && stats.soldBooks > 0 ? (
        ...
        <Container>
          <div className="max-w-4xl mx-auto flex flex-col gap-10">
            ...
            <ProfileHeader
              ...
              isSeller={hasSellActivity}
              statsNode={statsNode}
              badgeNode={sellerBadge}
              ...
              <>
                ...
                <div className="bg-surface border border-surface2 rounded-2xl px-6 py-5 flex items-center justify-between gap-4 flex-wrap">
                  <div>
                    ...
                    <p className="text-xs text-ash">Добавьте первую книгу — и ваш профиль станет витриной продавца</p>
                  </div>
                  <Link
                    href="/add-book"
