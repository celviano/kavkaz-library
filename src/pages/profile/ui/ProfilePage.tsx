...
export const ProfilePage = memo(() => {
  ...
  const fullName =
    getFullName(profile ?? null) || user.email?.split('@')[0] || 'Пользователь'
  ...
  const roleBadge = profile ? (
    <div className="flex items-center gap-2 mb-2 flex-wrap">
      <span
        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${ROLE_BADGE_STYLE[profile.role]}`}
      >
  ...
  const sellerExtraInfo =
    hasSellerRole && user.email ? (
      <ProfileInfoRow
  ...
  const trustNode =
    hasSellerRole && stats && stats.soldBooks > 0 ? (
                ...
                <div className="bg-surface border border-surface2 rounded-2xl px-6 py-5 flex items-center justify-between gap-4 flex-wrap">
                  <div>
                    <p className="text-sm font-medium text-ink mb-0.5">
                      Хотите продавать книги?
                    </p>
                    <p className="text-xs text-ash">
                      Добавьте первую книгу — профиль автоматически станет витриной
                      продавца
                    </p>
                  </div>
                  <Link
