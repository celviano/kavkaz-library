...
export const MyOrdersTab = memo<MyOrdersTabProps>(({ userId }) => {
  ...
  return (
    <ul className="flex flex-col gap-4">
      {orders.map((order) => {
        const totalPrice = order.bookPrice != null
          ...
          <li key={order.id}>
            <div className="bg-surface border border-surface2 rounded-2xl overflow-hidden">
              ...
              <div className="flex items-center justify-between gap-4 px-5 py-4 border-b border-surface2 flex-wrap">
                <div className="flex items-center gap-3">
                  ...
                  <div className="w-10 h-14 rounded-lg bg-surface2 overflow-hidden flex-shrink-0 relative">
                    {order.bookCoverUrl ? (
                      <Image src={order.bookCoverUrl} alt={order.bookTitle ?? ''} fill sizes="40px" className="object-cover"/>
                    ) : (
                      <div className="w-full h-full flex items-center justify-center">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9e9080" strokeWidth="1.5">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-ink truncate">{order.bookTitle ?? 'Книга удалена'}</p>
                    <p className="text-xs text-ash">
                      {order.quantity} экз.
                      {totalPrice != null && (
                        <> · <span className="text-accent font-medium">
                          {new Intl.NumberFormat('ru-RU', { style: 'currency', currency: order.bookCurrency, maximumFractionDigits: 0 }).format(totalPrice)}
                        </span></>
                      )}
                    </p>
                  </div>
              ...
              <div className="px-5 py-4 grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-3">
                ...
                <InfoRow label="Доставка"      value={order.deliveryType ? DELIVERY_LABELS[order.deliveryType] : null} />
                <InfoRow label="Город"         value={order.city} />
                <InfoRow label="Индекс"        value={order.postalCode} />
                <div className="col-span-2 sm:col-span-3">
                  <InfoRow label="Адрес"
                    value={[order.address, order.apartment ? `кв. ${order.apartment}` : null].filter(Boolean).join(', ')} />
                </div>
                {order.comment && (
                  <div className="col-span-2 sm:col-span-3">
