...
export const AddQuoteModal = memo<AddQuoteModalProps>(({ userId, onClose }) => {
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<AddQuoteValues>({
  ...
  function onSubmit(data: AddQuoteValues) {
    mutate({ text: data.text.trim(), author: data.author.trim(), source: data.source.trim() })
  }
        ...
        {isSuccess ? (
          <div className="flex flex-col items-center gap-5 px-6 py-12 text-center">
            <div className="w-16 h-16 rounded-full bg-accent/10 border border-accent/20 flex items-center justify-center">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#2a5c45" strokeWidth="2" strokeLinecap="round">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col overflow-y-auto">
            <div className="px-6 py-5 flex flex-col gap-4">
              <FormField label="Текст цитаты" required error={errors.text?.message}>
                <Textarea
                  ...
                  {...register('text')}
                />
                <p className="text-[11px] text-dim mt-1 text-right">{textLength}/600</p>
              </FormField>

              <FormField label="Автор" required error={errors.author?.message}>
                <Input placeholder="Александр Дюма" maxLength={150} {...register('author')} />
              </FormField>

              <FormField label="Источник" required error={errors.source?.message}>
                <Input placeholder="«Кавказ», 1859" maxLength={200} {...register('source')} />
              </FormField>
