...
export const RequestModal = memo<RequestModalProps>(({ book, sellerId, onClose }) => {
  ...
  const {
    ...
    mutationFn: (data: RequestOrderValues) =>
      createOrder({
        bookId: book.id,
        sellerId,
    ...
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center">
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-surface2 flex-shrink-0">
          <div>
            <p className="text-[10px] font-medium tracking-[2px] uppercase text-accent">
              Оформление заказа
            </p>
            <h2
              id="order-modal-title"
              className="text-base font-semibold text-ink leading-snug line-clamp-1 mt-0.5"
            >
              {book.title}
            </h2>
          </div>
        ...
        {isSuccess ? (
          ...
          <div className="flex flex-col items-center gap-4 py-12 px-6 text-center">
            ...
            <a
              href="/auth/login"
              className="h-11 px-8 rounded-xl text-sm font-medium bg-accent text-bg hover:bg-accent2 transition-all inline-flex items-center"
            >
              Войти
            </a>
          </div>
          ...
          <form
            onSubmit={handleSubmit((data) => sendOrder(data))}
            className="flex flex-col overflow-hidden"
          >
            <div className="overflow-y-auto px-6 py-5 flex flex-col gap-6">
              <section>
                <p className="text-[11px] font-medium tracking-[2px] uppercase text-accent mb-4">
                  Личные данные
                </p>
              ...
              <section>
                ...
                <Controller
                  ...
                  name="deliveryType"
                  render={({ field }) => (
                    <DeliveryPicker value={field.value} onChange={field.onChange} />
                  )}
                />
              </section>
